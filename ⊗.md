---
$: ⊗
$type: Morphism
$layer: ⭘
$aliases: [pair, tuple, cons, product]
$composition: primitive
---

# ⊗ — Pairing

> *"Two become a new One"*  
> *"Структура."*

---

## ∈ Формальне визначення

### Платонічна форма (λ-calculus)
Defined using Church Pairs:
```haskell
PAIR   ≡ λxyf.fxy
FIRST  ≡ λp.p(λxy.x)
SECOND ≡ λp.p(λxy.y)
```
A pair is a function that waits for a selector `f` and applies it to the stored values `x` and `y`.

### Type signature
```haskell
⊗ :: ∀α β. α → β → (α × β)
```

**Church-Rosser**: Confluent.

---

## [⊃](⊃) Властивості

### Algebraic Properties
1. **Projections**:
   - `first (pair x y) = x`
   - `second (pair x y) = y`
2. **Surjectivity**: Every element of `A × B` is a pair of some `a ∈ A` and `b ∈ B`.
3. **Bifunctoriality**: `map (f, g) (pair x y) = pair (f x) (g y)`.

### Computational Properties
- **Purity**: 1.0
- **Termination**: O(1).
- **Cost**: Allocation of structure.
- **Ports**: 3 (Input A, Input B, Output Pair).

---

## 📐 Hex-Torus Topology

### Visual Structure
```
      x     y
      │     │
    ┌─▼─────▼─┐
    │    ⊗    │
    └────┬────┘
         │
       (x,y)
```

The Pairing morphism is a **combiner**. It creates structure from individual elements.

### Geometric Properties
In the hexagonal tessellation:
- Occupies 1 hex cell.
- 2 inputs, 1 output.
- Bonding point.

### Sacred Geometry
```
      ¬
    /   \
   ∧     ?
  |   I   |
   ⊗     λ   ← The Structure (Left)
    \   /
      @
```
⊗ sits on the left, balancing Abstraction (`λ`) on the right.
Space (Memory) vs. Time (Computation).

---

## → Reduction Rules

### λ-reduction
```
(λxyf.fxy) a b
→ λf.fab
```
To access:
```
(λf.fab) (λxy.x)
→ (λxy.x) a b
→ a
```

---

## 🎭 Role in Composition

Pairing is the basis of **data structures**.
- **Lists**: Nested pairs (Cons cells).
- **Trees**: Pairs of pairs.
- **Contexts**: Storing state alongside value.

---

## 🔗 Зв'язки з іншими морфізмами

### With [λ](λ.md) (Abstraction)
Together they form the basis of most type systems.

### With [∧](∧.md) (AND)
Logical AND is a specific case of pairing where the values are booleans (Product Type).

---

## [⟶](⟶.md) Проекції

### TypeScript/Deno
```typescript
// Projection: ./𝟏.ts

// Native Tuple
export const pair = <A, B>(a: A, b: B): [A, B] => [a, b];

// Accessors
export const first = <A, B>(p: [A, B]): A => p[0];
export const second = <A, B>(p: [A, B]): B => p[1];

// Aliases
export const tuple = pair;
export const cons = pair;
```

### Rust (future)
```rust
pub fn pair<A, B>(a: A, b: B) -> (A, B) { (a, b) }
```

---

## 🔬 Докази

### Theorem 1: Projection
**Statement**: `first(pair(x, y)) = x`

**Proof**:
`pair(x, y)` creates `[x, y]`.
`first` accesses index 0.
`[x, y][0]` is `x`.
∎

---

## 🎯 Використання

### Returning Multiple Values
```typescript
const divMod = (n: number, d: number) => pair(Math.floor(n/d), n%d);
```

### Linked Lists
```typescript
const list = pair(1, pair(2, pair(3, null)));
```

---

## 🌌 Філософія

### [⚡️](⚡️.md)
Зберегти зв'язок. Створити простір для співіснування.

### Онтологія
- **Залежності**: Жодних (примітив).
- **Резонанс**: 90% (Relationship).
- **Метафора**: **Шлюб**. Два стають одним, залишаючись двома.

---

## 🔗 Посилання

- [Базовий шар (⭘)](⭘.md)
- [Abstraction (λ)](λ.md)
- [TypeScript проекція](./𝟏.ts)

---

*Проєкції можуть змінюватися. Зв'язок — вічний.*
