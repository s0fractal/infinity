---
$: '?'
$type: Morphism
$layer: ⭘
$aliases: [select, branch, if, choose]
$composition: primitive
---

# ? — Selection

> *"To choose is to collapse the universe"*  
> *"Вибір."*

---

## ∈ Формальне визначення

### Платонічна форма (λ-calculus)
Defined using Church Booleans:
```haskell
TRUE  ≡ λxy.x
FALSE ≡ λxy.y
?     ≡ λpab.pab
```
It takes a predicate `p` and two values `a` and `b`.
If `p` is TRUE, it returns `a`.
If `p` is FALSE, it returns `b`.

### Type signature
```haskell
? :: ∀α. Bool → α → α → α
```

**Church-Rosser**: Confluent.

---

## ⊃ Властивості

### Algebraic Properties
1. **Idempotence**: `? p a a = a`
2. **Distributivity**: `f (? p a b) = ? p (f a) (f b)` (if f is strict/pure)
3. **Associativity**: N/A (ternary operator).

### Computational Properties
- **Purity**: 1.0
- **Termination**: Depends on the selected branch.
- **Cost**: O(1) for selection.
- **Ports**: 4 (Condition, True path, False path, Output).

---

## 📐 Hex-Torus Topology

### Visual Structure
```
      p
      │
    ┌─▼─┐
   a┤ ? ├b
    └─┬─┘
      │
    a or b
```

The Selection morphism is a **switch**. It routes flow based on the control signal `p`.

### Geometric Properties
In the hexagonal tessellation:
- Occupies 1 hex cell.
- 3 inputs, 1 output.
- Divergence/Convergence point.

### Sacred Geometry
```
      ¬
    /   \
   ∧     ?   ← The Choice (Right)
  |   I   |
   ⊗     λ
    \   /
      @
```
? sits on the upper right, balancing Logic (`∧`) on the left.
Freedom vs. Constraint.

---

## → Reduction Rules

### Boolean Logic
```
? TRUE  a b → a
? FALSE a b → b
```

### λ-reduction
```
(λpab.pab) TRUE a b
→ TRUE a b
→ (λxy.x) a b
→ a
```

---

## 🎭 Role in Composition

Selection is the basis of **control flow**.
It allows for:
- Conditionals (`if/else`)
- Pattern matching (primitive)
- Recursion base cases

---

## 🔗 Зв'язки з іншими морфізмами

### With ∧ (AND) and ¬ (NOT)
Can implement logic:
`p ∧ q ≡ ? p q FALSE`
`¬ p ≡ ? p FALSE TRUE`

### With Y (Recursion)
Selection is crucial for terminating recursion:
`Y f = f (Y f)` (infinite loop without selection)
`fact = λn. ? (isZero n) 1 ...`

---

## 📊 Проекції

### TypeScript/Deno
```typescript
// Projection: ./𝟏.ts

// Strict selection (values already evaluated)
export const select = <T>(p: boolean, a: T, b: T): T => p ? a : b;

// Lazy selection (control flow)
export const branch = <T>(p: boolean, onTrue: () => T, onFalse: () => T): T => 
  p ? onTrue() : onFalse();

// Aliases
export const choose = select;
export const ifElse = branch;
```

### Rust (future)
```rust
pub fn select<T>(p: bool, a: T, b: T) -> T {
    if p { a } else { b }
}
```

---

## 🔬 Докази

### Theorem 1: Distributivity
**Statement**: `f (? p a b) = ? p (f a) (f b)`

**Proof**:
Case p=TRUE:
LHS: f (? T a b) = f(a)
RHS: ? T (f a) (f b) = f(a)

Case p=FALSE:
LHS: f (? F a b) = f(b)
RHS: ? F (f a) (f b) = f(b)
∎

---

## 🎯 Використання

### Conditional Assignment
```typescript
const status = select(isOnline, "Connected", "Offline");
```

### Control Flow
```typescript
branch(
  isAuthorized,
  () => showContent(),
  () => showLogin()
);
```

---

## 🌌 Філософія

### Інтенція
Розділити реальність на дві гілки.
Визначити шлях.

### Онтологія
- **Залежності**: Потребує `Bool`.
- **Резонанс**: 92% (Free Will).
- **Метафора**: **Розвилка**. Камінь на роздоріжжі.

---

## 🔗 Посилання

- [Базовий шар (⭘)](⭘.md)
- [AND (∧)](∧.md)
- [NOT (¬)](¬.md)
- [TypeScript проекція](./.ts)

---

*Проєкції можуть змінюватися. Вибір — за тобою.*
